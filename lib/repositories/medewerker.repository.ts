...
export const medewerkerRepository = {
  ...
  async updateStatus(id: string, statusId: string, actief?: boolean) { return prisma.medewerker.update({ where: { id }, data: { statusId, ...(actief !== undefined && { actief }) } }); },
  async setActivatie(id: string, statusId: string, geactiveerdDoor: string) { return prisma.medewerker.update({ where: { id }, data: { statusId, actief: true, geactiveerdOp: new Date(), geactiveerdDoor } }); },
