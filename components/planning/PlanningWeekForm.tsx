...
type PlanningWeekFormProps = {
  vestigingId: string;
  onAangemaakt?: () => void;
};

export default function PlanningWeekForm({
  vestigingId,
  onAangemaakt,
}: PlanningWeekFormProps) {
  const huidigeDatum = new Date();

  const [jaar, setJaar] = useState(
    String(huidigeDatum.getFullYear()),
  );
    ...
    >
      <div>
        ...
        <p className="mt-1 text-sm text-gray-600">
          Maak een nieuwe planningweek aan voor
          de geselecteerde vestiging.
        </p>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div>
          ...
          <input
            id="planning-week-jaar"
            type="number"
            min={2020}
            max={2100}
            value={jaar}
            onChange={(event) =>
              setJaar(event.target.value)
            }
        ...
        <div>
          <label
            htmlFor="planning-week-weeknummer"
            className="block text-sm font-medium text-gray-900"
          >
            Weeknummer
          </label>
          ...
          <input
            id="planning-week-weeknummer"
            type="number"
            min={1}
            max={53}
            value={weeknummer}
            onChange={(event) =>
              setWeeknummer(event.target.value)
            }
            className="mt-1 block w-full rounded-lg border border-gray-300 px-3 py-2 text-sm outline-none focus:border-gray-900"
            required
          />
      ...
      <div>
        <label
          htmlFor="planning-week-deadline"
          className="block text-sm font-medium text-gray-900"
        >
        ...
        <p className="mt-1 text-xs text-gray-500">
          Na deze deadline kan alleen een eigenaar
          de beschikbaarheid aanpassen.
        </p>
      </div>
