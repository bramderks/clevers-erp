...
export async function PATCH(request:Request,{params}:{params:Promise<{id:string;vakantieId:string}>}){
 ...
 const aanvraag=await prisma.vakantieAanvraag.findFirst({where:{id:vakantieId,medewerkerId:id},select:{id:true,vestigingId:true,vestiging:{select:{organisatieId:true}}}});
 ...
 if(!eigenaar)return NextResponse.json({fout:"Alleen de eigenaar van deze organisatie mag vakantieaanvragen beoordelen."},{status:403});
