...
function sterkeWachtwoord(wachtwoord: string) { return wachtwoord.length >= 12 && /[a-z]/.test(wachtwoord) && /[A-Z]/.test(wachtwoord) && /\d/.test(wachtwoord) && /[^A-Za-z0-9]/.test(wachtwoord); }
export async function POST(request: Request) {
  try {
    ...
    const resultaat = await prisma.$transaction(async (tx) => {
      const systeemGebruiker = await tx.systeemGebruiker.create({ data: { naam: `${uitnodiging.voornaam} ${uitnodiging.achternaam}`, email: uitnodiging.email, wachtwoordHash, actief: false } });
      const medewerker = await tx.medewerker.create({ data: { systeemGebruikerId: systeemGebruiker.id, aanhef, voornaam: uitnodiging.voornaam, achternaam: uitnodiging.achternaam, roepnaam: roepnaam || null, geboortedatum: new Date(`${geboortedatum}T00:00:00`), email: uitnodiging.email, telefoon, statusId: status.id, actief: false, geactiveerdOp: new Date(), geactiveerdDoor: "MEDEWERKER" } });
    ...
    return NextResponse.json({ error: error instanceof Error && error.message.includes("Unique constraint") ? "Dit account is al geactiveerd of het e-mailadres is al in gebruik." : "Het account kon niet worden geactiveerd." }, { status: 500 });
  }
}
