...
export default function MedewerkerDocumentenPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function upload(event: FormEvent) {
    ...
    data.set("verloopDatum", verloopDatum);
    data.set("opmerkingen", opmerkingen);

    const response = await fetch("/api/medewerkers/" + id + "/documenten/upload", { method: "POST", body: data });
    const result = await response.json();

    if (!response.ok) {
      setBericht(result.fout ?? "Upload mislukt.");
    } else {
      setBericht("Document succesvol geüpload.");
      setFile(null);
      setVerloopDatum("");
      setOpmerkingen("");
      await laden();
  ...
  return (
    <main className="mx-auto max-w-3xl space-y-6 p-4 md:p-8">
      <header>
        <p className="text-sm text-slate-500">Eigenaar · Medewerkerbeheer</p>
        <h1 className="text-2xl font-bold text-slate-900">Documenten uploaden</h1>
        <p className="mt-1 text-sm text-slate-600">PDF, JPG, PNG of WEBP tot maximaal 10 MB.</p>
      </header>
      ...
      <form onSubmit={upload} className="space-y-4 rounded-2xl border bg-white p-5 shadow-sm">
        ...
        <select value={categorie} onChange={(e) => setCategorie(e.target.value)} className="w-full rounded-xl border p-3">
          <option>Certificaat</option>
        ...
        <button disabled={bezig || !file} className="rounded-xl bg-slate-900 px-5 py-3 font-semibold text-white disabled:opacity-50">
          {bezig ? "Uploaden..." : "Document uploaden"}
        </button>
      </form>
      ...
      <section className="rounded-2xl border bg-white shadow-sm">
        <div className="border-b p-5"><h2 className="font-bold">Documenten</h2></div>
        {documenten.length === 0 ? <p className="p-5 text-sm text-slate-500">Nog geen documenten.</p> : (
          <div className="divide-y">
            {documenten.map((document) => (
              <div key={document.id} className="flex flex-wrap items-center justify-between gap-3 p-5">
                <div><p className="font-semibold">{document.naam}</p><p className="text-sm text-slate-500">{document.categorie}{document.verloopDatum ? " · verloopt " + new Intl.DateTimeFormat("nl-NL",{dateStyle:"medium"}).format(new Date(document.verloopDatum)) : ""}</p></div>
                <div className="flex gap-2">
                  <a href={document.url} target="_blank" rel="noreferrer" className="rounded-lg border px-3 py-2 text-sm">Openen</a>
                  <button onClick={() => void verwijderen(document.id)} className="rounded-lg border px-3 py-2 text-sm">Verwijderen</button>
                </div>
              </div>
            ))}
          </div>
