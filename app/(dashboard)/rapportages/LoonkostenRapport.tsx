  ...
  return (
    <section className="space-y-5">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">
              ...
              {p.gemiddeldeSeizoensgroei != null && <>{" · "}groeifactor prognose: <strong className="text-slate-800">{(p.gemiddeldeSeizoensgroei * 100).toFixed(1).replace(".", ",")}%</strong></>}
            ...
            <div className="max-h-[calc(90vh-73px)] overflow-y-auto p-5 space-y-3">
              {p.diensten.map((dienst, i) => (
                <div key={i} className="rounded-2xl border border-slate-200 p-4">
                  <div className="flex justify-between gap-3"><div><p className="font-semibold text-slate-900">{datum(dienst.datum.slice(0,10))}</p><p className="text-sm text-slate-500">{new Intl.DateTimeFormat("nl-NL",{hour:"2-digit",minute:"2-digit",hour12:false,timeZone:"Europe/Amsterdam"}).format(new Date(dienst.begintijd))} – {new Intl.DateTimeFormat("nl-NL",{hour:"2-digit",minute:"2-digit",hour12:false,timeZone:"Europe/Amsterdam"}).format(new Date(dienst.eindtijd))}</p></div><span className="text-sm text-slate-500">{dienst.medewerkers.length} medewerker{dienst.medewerkers.length === 1 ? "" : "s"}</span></div>
      ...
      {detail === "omzet" && (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-slate-950/40 sm:items-center sm:p-6" onMouseDown={() => setDetail(null)}>
          <div className="max-h-[90vh] w-full max-w-3xl overflow-hidden rounded-t-3xl bg-white shadow-2xl sm:rounded-3xl" onMouseDown={e => e.stopPropagation()}>
            <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4"><div><h2 className="text-lg font-bold text-slate-900">Omzet · seizoen per week</h2><p className="text-xs text-slate-500">Afgesloten weken zijn gerealiseerd; toekomstige weken worden automatisch geraamd op basis van de gemiddelde groei van het vorige seizoen t.o.v. het seizoen daarvoor.</p></div><button onClick={() => setDetail(null)} className="rounded-xl p-2 text-slate-400 hover:bg-slate-100"><X size={20}/></button></div>
            <div className="max-h-[calc(90vh-73px)] overflow-y-auto p-5">
              ...
              <div className="space-y-1">{p.seizoenWeken.map(w => { const vorig=vorigeMap.get(`${w.jaar}-${w.weeknummer}`); return <div key={`${w.jaar}-${w.weeknummer}`} className="grid grid-cols-[56px_1fr_1fr] items-center gap-3 rounded-xl px-3 py-3 text-sm hover:bg-slate-50"><span className="font-semibold">W{w.weeknummer}</span><span>{w.verwachteOmzet == null ? "—" : euro(w.verwachteOmzet)} <small className="text-slate-400">{w.afgesloten ? "· werkelijk" : w.prognoseBron === "handmatig" ? "· handmatige verwachting" : w.prognoseBron === "vorig_seizoen_groei" ? "· prognose op groei" : "· verwachting"}</small></span><span className="text-right text-slate-500">{vorig?.omzet == null ? "—" : euro(vorig.omzet)}</span></div>})}</div>
