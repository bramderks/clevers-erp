...
export async function medewerkerAanmelden(
  ...
  const gegevens = medewerkerAanmeldenValidator.parse(dto);

  const emailBestaat = await bestaatEmail(gegevens.email);

  if (emailBestaat) {
    throw new Error("Er bestaat al een medewerker met dit e-mailadres.");
  }
...
module: "MEDEWERKER",
  ...
  const wachtwoordHash = await bcrypt.hash(
    gegevens.wachtwoord,
    12,
  );

  const medewerker = await medewerkerAanmaken({
    aanhef: gegevens.aanhef,
    voornaam: gegevens.voornaam,
    tussenvoegsel: gegevens.tussenvoegsel || null,
    achternaam: gegevens.achternaam,
    roepnaam: gegevens.roepnaam || null,
    geboortedatum: new Date(gegevens.geboortedatum),
